...
const shapes = computed(() => {
  ...
  return rollResult.value.map((roll) => roll.value).filter((value) => value >= 3).sort().reverse();
});
...
<template>
  <main>
    ...
    <div v-else class="roller-interface">
      ...
      <div class="dice-pool">
        <SkillDie v-for="(dieType, index) in pool.assembledTypes" :key="index" :die-type="dieType"
